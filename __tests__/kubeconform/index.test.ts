...
import { ToolRunner } from '@actions/exec/lib/toolrunner.js';
...
describe('Kubeconform', () => {
  ...
  test.each([
    ['arm64', 'arm64'],
    ['x64', 'amd64'],
  ])('getKubeconformArch() - return on %s os arch %s kubeconform arch', (osArch, kubeconformArch) => {
  ...
  test.each([['arm64'], ['amd64']])('returns url to download kubeconform for Linux %s', (arch) => {
    ...
    expect(kubeconform.getKubeconformDownloadUrl(arch)).toBe(kubeconformLinuxUrl);
    expect(os.type).toHaveBeenCalled();
  });

  test.each([['arm64'], ['amd64']])('returns url to download kubeconform for Darwin %s', (arch) => {
  ...
  test.each([['arm64'], ['amd64']])('returns url to download kubeconform for Windows %s', (arch) => {
  ...
  test('downloads kubeconform, extract zip, and returns path to it', async () => {
    ...
    expect(await kubeconform.downloadKubeconform()).toBe(path.join('pathToExtractedTool', 'kubeconform.exe'));
    expect(os.type).toHaveBeenCalled();
    expect(toolCache.downloadTool).toHaveBeenCalled();
    expect(toolCache.extractZip).toHaveBeenCalledWith('pathToTool');
    expect(fs.chmodSync).toHaveBeenCalledWith(path.join('pathToExtractedTool', 'kubeconform.exe'), '755');
  ...
  test('downloads kubeconform, extract tar, and returns path to it', async () => {
    ...
    expect(await kubeconform.downloadKubeconform()).toBe(path.join('pathToExtractedTool', 'kubeconform'));
    expect(os.type).toHaveBeenCalled();
    expect(toolCache.downloadTool).toHaveBeenCalled();
    expect(io.cp).toHaveBeenCalledWith(path.join('pathToToolDir', 'tool'), path.join('pathToToolDir', 'tool.tar.gz'));
    expect(toolCache.extractTar).toHaveBeenCalledWith(path.join('pathToToolDir', 'tool.tar.gz'));
    expect(fs.chmodSync).toHaveBeenCalledWith(path.join('pathToExtractedTool', 'kubeconform'), '755');
    expect(core.addPath).toHaveBeenCalledWith('pathToExtractedTool');
  });

  test('throws error if download tool fails', async () => {
    jest.spyOn(os, 'type').mockReturnValue('Windows_NT');
    jest.spyOn(toolCache, 'downloadTool').mockRejectedValue(new Error('Unable to download'));
  ...
  test('Gets path to kubeconform and uses it on manifests', async () => {
    ...
    expect(await kubeconform.kubeconformLint(sampleManifests, kubeconformOpts)).toBeUndefined();
    expect(io.which).toHaveBeenCalledWith('kubeconform', false);
    expect(ToolRunner).toHaveBeenCalledWith('pathToTool', ['-summary', 'manifest1.yaml']);
    expect(ToolRunner).toHaveBeenCalledWith('pathToTool', ['-summary', 'manifest2.yaml']);
    expect(ToolRunner).toHaveBeenCalledWith('pathToTool', ['-summary', 'manifest3.yaml']);
  ...
  test('check if kubeconform is already installed, else download, and use it on manifests', async () => {
    ...
    expect(await kubeconform.kubeconformLint(sampleManifests, kubeconformOpts)).toBeUndefined();
    expect(io.which).toHaveBeenCalledWith('kubeconform', false);
    expect(ToolRunner).toHaveBeenCalledWith(path.join('pathToExtractedTool', 'kubeconform.exe'), [
      '-summary',
      'manifest1.yaml',
    ]);
    expect(ToolRunner).toHaveBeenCalledWith(path.join('pathToExtractedTool', 'kubeconform.exe'), [
  ...
  test('throw if kubeconform fails on a manifest', async () => {
    ...
    await expect(kubeconform.kubeconformLint(sampleManifests, kubeconformOpts)).rejects.toThrow();
    expect(io.which).toHaveBeenCalledWith('kubeconform', false);
    expect(ToolRunner).toHaveBeenCalledWith('pathToTool', ['-summary', 'manifest1.yaml']);
