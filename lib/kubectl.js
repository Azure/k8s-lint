...
function downloadKubectl(version) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!cachedToolpath) {
            ...
            cachedToolpath = yield toolCache.cacheFile(kubectlDownloadPath, kubectlToolName + utils_1.getExecutableExtension(), kubectlToolName, version);
        }
        const kubectlPath = path.join(cachedToolpath, kubectlToolName + utils_1.getExecutableExtension());
        fs.chmodSync(kubectlPath, '777');
        return kubectlPath;
    });
}
function kubectlEvalLint(manifests) {
    return __awaiter(this, void 0, void 0, function* () {
        let toolPath = yield downloadKubectl(yield getStableKubectlVersion());
        for (let i = 0; i < manifests.length; i++) {
            const manifest = manifests[i];
            let toolRunner = new toolrunner_1.ToolRunner(toolPath, ['apply', '-f', manifest, '--server-dry-run']);
            yield toolRunner.exec();
        }
    });
}
exports.kubectlEvalLint = kubectlEvalLint;
